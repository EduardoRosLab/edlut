...
std::map<std::string, boost::any> LIFTimeDrivenModel_IS_GPU_C_INTERFACE::GetSpecificNeuronParameters(int index) const noexcept(false){
	return GetParameters();
}

void LIFTimeDrivenModel_IS_GPU_C_INTERFACE::SetParameters(std::map<std::string, boost::any> param_map) noexcept(false){
...
IntegrationMethod_GPU_C_INTERFACE * LIFTimeDrivenModel_IS_GPU_C_INTERFACE::CreateIntegrationMethod(ModelDescription imethodDescription) noexcept(false){
...
ModelDescription LIFTimeDrivenModel_IS_GPU_C_INTERFACE::ParseNeuronModel(std::string FileName) noexcept(false){
	FILE *fh;
