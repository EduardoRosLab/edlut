...
#ifndef LIFTIMEDRIVENMODEL_GPU2_H_
#define LIFTIMEDRIVENMODEL_GPU2_H_
...
class LIFTimeDrivenModel_GPU2 : public TimeDrivenNeuronModel_GPU2 {
	public:
		/*!
		...
		*/
		 ...
		 *
		 */
		__device__ LIFTimeDrivenModel_GPU2(float e_exc, float e_inh, float e_leak, float v_thr, float c_m, float tau_exc, float tau_inh,
				float tau_ref, float g_leak, float tau_nmda, char const* integrationName, int N_neurons, void ** Buffer_GPU):TimeDrivenNeuronModel_GPU2(MilisecondScale_GPU),
				e_exc(e_exc), e_inh(e_inh), e_leak(e_leak), v_thr(v_thr), c_m(c_m), inv_c_m(1.0f/c_m), tau_exc(tau_exc), inv_tau_exc(1.0f / tau_exc),
				tau_inh(tau_inh), inv_tau_inh(1.0f/tau_inh), tau_ref(tau_ref), g_leak(g_leak), tau_nmda(tau_nmda), inv_tau_nmda(1.0f/tau_nmda){

			this->integration_method_GPU2 = IntegrationMethodFactory_GPU2<LIFTimeDrivenModel_GPU2>::loadIntegrationMethod_GPU2(integrationName, Buffer_GPU, this);
		 ...
		 */
		__device__ virtual ~LIFTimeDrivenModel_GPU2(){
		...
		__device__ void EvaluateDifferentialEquation(int index, int SizeStates, float * NeuronState, float * AuxNeuronState, float elapsed_time){
			...
			float ext_I = NeuronState[4*SizeStates + index];
			...
			if (vectorNeuronState_GPU2->LastSpikeTimeGPU[index] * this->GetTimeScale()>this->tau_ref){
				...
				AuxNeuronState[blockDim.x*blockIdx.x + threadIdx.x]=(current + g_leak * (this->e_leak - V))*this->inv_c_m;
			}
				...
				AuxNeuronState[blockDim.x*blockIdx.x + threadIdx.x]=fraction*(current + g_leak * (this->e_leak - V))*this->inv_c_m;
			}else{
