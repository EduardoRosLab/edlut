...
NeuronModelTable::NeuronModelTable(): OwnerType(0), elems(0), ndims(0), nelems(0), interp(0), firstintdim(0){
...
const NeuronType * NeuronModelTable::GetOwnerType() const{
	return this->OwnerType;
}
  		
void NeuronModelTable::SetOwnerType(NeuronType * Owner){
...
// Direct table access for interpolated tables
float NeuronModelTable::TableAccessDirectInterp(float statevars[MAXSTATEVARS]){
	int idim,tind,tableind;
		...
		coord=statevars[dim->statevar];
			...
			if(coord>last_coord(dim)){
				...
				if(coord<dim->vfirst){
					...
					tableind=table_ind_int(dim,coord);
               				if(((coord-coords[tableind])/(coords[tableind+1]-coords[tableind])) > 0.5){
						tind=tableind+1;
					}else{
						tind=tableind;
					}
